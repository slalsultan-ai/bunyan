...
interface Question {
  ...
  difficulty: string; questionType: string; questionTextAr: string; isActive: boolean; createdAt: string;
}
...
export default function QuestionsPage() {
  ...
  const load = useCallback(async (p = page) => {
    setLoading(true);
    const params = new URLSearchParams({ page: String(p), ...(filters.age_group && { age_group: filters.age_group }), ...(filters.skill_area && { skill_area: filters.skill_area }), ...(filters.difficulty && { difficulty: filters.difficulty }), ...(filters.type && { type: filters.type }), ...(filters.active !== 'all' && { active: filters.active }), ...(filters.search && { search: filters.search }) });
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-4">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          ...
          {[
            { key: 'age_group', opts: AGES, label: 'الفئة العمرية', labels: {} },
